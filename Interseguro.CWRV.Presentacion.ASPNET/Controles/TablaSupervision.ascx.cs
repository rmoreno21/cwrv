...
{
    public partial class TablaSupervision : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (PermisoConsultar)
            {
                TabSupervision.DataSource = DatosReporte;
                TabSupervision.DataBind();

                if (TotalRegistros > 0)
                {
                    TabSupervision.HeaderRow.TableSection = TableRowSection.TableHeader;
                    TabSupContenedorPaginador.Visible = true;
                }
                ...
                {
                    string paginador;
                    paginador = "<ul id=\"TabSupervisionPaginador\" class=\"tsc_pagination tsc_paginationB tsc_paginationB02\">";
                    if (IndicePagina > 1)
                    {
                    ...
                    TabSupervisionPaginador.Text = paginador;

                    TabSupervisionPaginadorIndice.Text = "Página " + IndicePagina + "/" + totalPaginas;
                }
            }
            else
            {
        ...
        protected void TabSupervision_RowDataBound(object sender, GridViewRowEventArgs e)
        {
