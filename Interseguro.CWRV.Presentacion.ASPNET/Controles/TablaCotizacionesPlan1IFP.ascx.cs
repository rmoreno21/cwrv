...
namespace Interseguro.CWRV.Presentacion.ASPNET.Controles
{
    ...
    public partial class TablaCotizacionesPlan1IFP : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            TabCotizaciones_IFP_P1.DataSource = CotizacionesIFP;
            TabCotizaciones_IFP_P1.DataBind();

            if (CotizacionesIFP.Count > 0)
            {
                TabCotizaciones_IFP_P1.HeaderRow.TableSection = TableRowSection.TableHeader;
            }
            ...
            TabCotizaciones_IFP_P1.Columns[8].Visible = Utilitarios.ValidarRedLocal(Request.UserHostAddress);

            TabCotizaciones_IFP_P1.Columns[12].Visible = PermisoEspeciales;
            TabCotizaciones_IFP_P1.Columns[13].Visible = PermisoEspeciales;
            TabCotizaciones_IFP_P1.Columns[14].Visible = PermisoEspeciales;
            TabCotizaciones_IFP_P1.Columns[15].Visible = PermisoTRA;
            ...
            {
                TabCotizaciones_IFP_P1.Columns[16].Visible = false;
                TabCotizacionesBotonera_IFP.Visible = false;
            }
        ...
        protected void TabCotizaciones_RB_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (Convert.ToInt32(TabCotTemporalidad.SelectedItem.Text) == 15)
                {
                    lstPerDiferido = lstPerDiferido.FindAll(pd => Convert.ToDouble(pd.Nombre) <= 7.5);
                }
                else if (Convert.ToInt32(TabCotTemporalidad.SelectedItem.Text) <= 10)
                {
                    lstPerDiferido = lstPerDiferido.FindAll(pd => Convert.ToDouble(pd.Nombre) <= 5);
                }
                else
                {
                    lstPerDiferido = lstPerDiferido.FindAll(pd => Convert.ToDecimal(pd.Nombre) <= Convert.ToInt32(TabCotTemporalidad.SelectedItem.Text));
                }
                ...
                lstDevolucion = lstDevolucion.Where(t => t.Valor_2.Contains(Enums.Planes.PLAN1.StringValue())).ToList();
                ...
                //Combobox Fallecimiento
                DropDownList TabCotDevolucionFallecimiento;
                TabCotDevolucionFallecimiento = (DropDownList)(fila.FindControl("TabCotDevolucionFallecimiento"));
                TabCotDevolucionFallecimiento.ClearSelection();
                List<Parametro> lstFallecimiento = (List<Parametro>)Session["ComboIFPDevFallecimiento"];

                if (TabCotDevolucion.Text == "0")
                {
                    lstFallecimiento = lstFallecimiento.FindAll(pd => pd.Id == "0");
                }
                else
                {
                    lstFallecimiento = lstFallecimiento.FindAll(pd => Convert.ToInt32(pd.Id) <= Convert.ToInt32(TabCotDevolucion.SelectedItem.Value));
                }

                CargarParametros(TabCotDevolucionFallecimiento, lstFallecimiento);
                TabCotDevolucionFallecimiento.SelectedIndex = TabCotDevolucionFallecimiento.Items.IndexOf(TabCotDevolucionFallecimiento.Items.FindByValue(CotizacionesIFP[e.Row.RowIndex].ValPjeDevFallec.ToString()));
                ...
                if (CotizacionesIFP[e.Row.RowIndex].Temporalidad.Anhos == CotizacionesIFP[e.Row.RowIndex].ValPerDiferido)
                {
                    ...
                    TabCotDevolucionFallecimiento.Enabled = false;
                ...
                if (ModoSolicitud != null && ModoSolicitud.ToString() == "CONS")
                {
                    ...
                    TabCotizaciones_IFP_P1.Columns[16].Visible = false;
                    TabCotizacionesBotonera_IFP.Visible = false;
                    TabCotizaciones_IFP_P1.Enabled = false;
                }
