...
{
    public partial class TablaCotizacionesRentaPrivadaPlus : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!PermisoAgregar || CotizacionesRPPlus.Count >= Convert.ToInt32(ConfigurationManager.AppSettings["MaxCotizaciones"]))
            {
                TabCotizacionesBotonera_RP.Visible = false;
            }

            TabCotizaciones_RP.Columns[8].Visible = Conyuge;
            TabCotizaciones_RP.Columns[13].Visible = PermisoEspeciales;
            TabCotizaciones_RP.Columns[14].Visible = PermisoEspeciales;
            TabCotizaciones_RP.Columns[15].Visible = PermisoEspeciales;
            TabCotizaciones_RP.Columns[16].Visible = PermisoTRA;

            var CotizacionSeleccionada = CotizacionesRPPlus.FindAll(p => p.IndSeleccionada == "S" && p.EstadoCotizacion == "04");
        ...
        protected void TabCotizaciones_RB_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //if (CotizacionesRPPlus[e.Row.RowIndex].IndGastoSepelio == "S")
                //{
                ...
                TabCotMoneda.SelectedIndex = TabCotMoneda.Items.IndexOf(TabCotMoneda.Items.FindByValue(CotizacionesRPPlus[e.Row.RowIndex].Moneda.Id));
                ...
                TabCotPeriodoGarantizado.SelectedIndex = TabCotPeriodoGarantizado.Items.IndexOf(TabCotPeriodoGarantizado.Items.FindByValue(CotizacionesRPPlus[e.Row.RowIndex].PeriodoGarantizado.ToString()));
                ...
                TabCotPagoEscalonada.SelectedIndex = TabCotPagoEscalonada.Items.IndexOf(TabCotPagoEscalonada.Items.FindByValue(CotizacionesRPPlus[e.Row.RowIndex].PagoEscalonada.ToString()));
                ...
                //CargarComboboxconValor(TabCotPjePagoEscalonada, (List<Parametro>)Session["ComboPorcentajeEscalonada"]);
                //TabCotPjePagoEscalonada.SelectedIndex = TabCotPjePagoEscalonada.Items.IndexOf(TabCotPjePagoEscalonada.Items.FindByValue(CotizacionesRPPlus[e.Row.RowIndex].PjePE.ToString()));
                ...
                var ValorEscalon = "0";
                if (CotizacionesRPPlus[e.Row.RowIndex].PjePE > 0)
                {
                    ValorEscalon = lstTramoEscalonada.ToList().Find(esc => (((Convert.ToDouble(esc.Valor_1) - 0.000000001) * 100) <= (CotizacionesRPPlus[e.Row.RowIndex].PjePE)) && (((Convert.ToDouble(esc.Valor_1) + 0.000000001) * 100) >= (CotizacionesRPPlus[e.Row.RowIndex].PjePE))).Id;
                }
                ...
                //<INI.GTI_15930>
                //List<Parametro> lstPorcentajeDevolucion = (List<Parametro>)Session["ComboPorcentajeDevolucion"];
                List<Parametro> lstPorcentajeDevolucion = (List<Parametro>)Session["ComboPorcentajeDevolucion"];

                List<Parametro> lstPorcentajeDevolucion2 = new List<Parametro>();
                foreach (var item in lstPorcentajeDevolucion)
                {
                    if (Moneda == Enums.Moneda.Soles.StringValue())
                    {
                        if (Temporalidad == 5 || Temporalidad == 7)
                        {
                            if (item.Id != "25")
                            {
                                lstPorcentajeDevolucion2.Add(item);
                            }
                        }
                        else
                        {
                ...
                CargarCombobox(TabCotPjeDevolucion, lstPorcentajeDevolucion2, false);
                TabCotPjeDevolucion.SelectedIndex = TabCotPjeDevolucion.Items.IndexOf(TabCotPjeDevolucion.Items.FindByValue(CotizacionesRPPlus[e.Row.RowIndex].ValPjeDev.ToString()));

                TextBox TabCotTRA;
                TabCotTRA = (TextBox)(fila.FindControl("TabCotTRA"));
                TabCotTRA.Text = CotizacionesRPPlus[e.Row.RowIndex].AjusteTRA.ToString();
                ...
                TabCotPjeConyuge.SelectedIndex = TabCotPjeConyuge.Items.IndexOf(TabCotPjeConyuge.Items.FindByValue(CotizacionesRPPlus[e.Row.RowIndex].ValPjeConyuge.ToString()));
                ...
                if (lstParametro.Count > 0)
                {
                    ...
                    TabCotAjusteMoneda.SelectedIndex = TabCotAjusteMoneda.Items.IndexOf(TabCotAjusteMoneda.Items.FindByValue(CotizacionesRPPlus[e.Row.RowIndex].ValMonAju.ToString()));
                }
                //<FINGTI_753>


                //<INIGTI_753_3>
                var modSolModo = HttpContext.Current.Session["ModSolModo"];

                if (modSolModo != null && modSolModo.ToString() == "CONS")
                {
                    ...
                    TabCotTRA.CssClass = TabCotTRA.CssClass + " formTextboxReadOnly";
