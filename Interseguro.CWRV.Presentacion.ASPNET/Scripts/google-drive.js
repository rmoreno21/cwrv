...
****/
    ...
    var API_KEY = '';
    var FOLDER_NAME = "";
    var FOLDER_ID = "root";//"1rGrbwmYaR-6PTiwhqgZe5S1scu65joKF";//"root";
    ...
    function initClient() {
        ...
        //FOLDER_ID = "1rGrbwmYaR-6PTiwhqgZe5S1scu65joKF";
        gapi.client.init({
            ...
            if ($('#HEstado').val() == "0")
                bOcultarBotonDrive = false;
            if ($('#HEstado').val() == "1")
                bOcultarBotonDrive = false;
            else if ($('#HEstadoPlaft').val() == "1" && ($('#HEstado').val() == "5" || $('#HEstado').val() == "6"))
                bOcultarBotonDrive = false;
            else if ($('#HEstado').val() == "5" && ($('#HEstadoPlaft').val() == "3" || $('#HEstadoPlaft').val() == "1"))
                bOcultarBotonDrive = false;

            if (bOcultarBotonDrive == true)//
                $('.button-opt').hide();
            else
                $('.button-opt').show();
            ...
            gapi.client.load('drive', 'v2', crearCarpeta);
            ...
            if (!resp.error) {
                ...
                if (DRIVE_FILES.length > 0) {
                    for (var i = 0; i < DRIVE_FILES.length; i++) {
                        if (DRIVE_FILES[i].title == $("#HNroSolicitud").val()) {
                            
                            bExisteCarpeta = true;
                            FOLDER_ID = DRIVE_FILES[i].id;// resp.id;
                            break;
                        }
                    }
    ...
    $(function () {
        ...
        $("#fUpload").bind("change", function () {
            ...
            var files = document.getElementById("fUpload").files;
            ...
            for (var i = 0; i < files.length; i++) {
                var file = files[i];
                var file1 = uploadObj.prop("files")[i];
                var metadata = {
                    'title': file.name,
                    'description': "Cotizador Web de Rentas - CWRV",
        ...
        if (DRIVE_FILES.length > 0) {
            for (var i = 0; i < DRIVE_FILES.length; i++) {
                ...
                if (DRIVE_FILES[i].fileType != "file") {
                    fText += "<div class='folder-icon' data-level='" + DRIVE_FILES[i].level + "' data-parent='" + DRIVE_FILES[i].parentID + "' data-size='" + DRIVE_FILES[i].fileSize + "' data-id='" + DRIVE_FILES[i].id + "' title='" + textTitle + "' data-name='" + DRIVE_FILES[i].title + "' data-has-permission='" + DRIVE_FILES[i].hasPermission + "'><div class='image-preview'><img src='../Estilos/images/folder.png'/></div></div>";
                } else {
    ...
    function initDriveButtons() {
        ...
        $(".button-text").click(function () {
            ...
            setTimeout(function () {
                ...
                //If there is a text version, we get this version instead.
                if (DRIVE_FILES[FILE_COUNTER].alternateLink == null) {
                    
                    window.open(DRIVE_FILES[FILE_COUNTER]['exportLinks']['text/plain']);
                } else {
                    
                    window.open(DRIVE_FILES[FILE_COUNTER].alternateLink);
                }
                hideLoading();
                hideStatus();
            }, 1000);
