...
$(document).ready(function () {
    ...
    $('.grilla_editar').live('click', function () {
        ...
        postBack("GrupoFamiliarAfiliadoCierre.aspx?indiceBeneficiario=" + idGrupoFamiliar + "&form=" + "CA");
    });

    $('#ModSiguiente').live('click', function () {
        var validaBenCA = true;
        if ($('#HSolicitudSerializado') != undefined) {
            ...
            for (var b in lstBeneficiarios) {
                if (lstBeneficiarios[b].TipoCobertura == 'CA') {
                    if (lstBeneficiarios[b].Identificacion == null) validaBenCA = false;
                }
            }
        }
        if (validaBenCA) {
            postBack('ResumenBeneficiariosPG.aspx');
        }
        else {
            $('#MCMIcono').attr('class', 'validacion');
            $('#MCMContenedor').html('Debe ingresar la información completa para todos los beneficiarios de Clausula Adicional');
    ...
    $('#ModCancelar').live('click', function () {
...
function CargarTablaBeneficiariosCierre_IFP() {
    ...
    $.ajax({
        type: 'POST',
        url: 'ResumenBeneficiariosCA.aspx/CargarTablaBeneficiarios',
        contentType: "application/json; charset=iso-8859-1",
        dataType: 'json',
        data: $.toJSON(params),
        success: function (data) {
            if (data.d[0] != 'TOKEN') {
                ...
                $('#TablaBeneficiariosContenedor_IFP').html($(data.d[0]).find('#ContenidoDinamico').html());

                $('#HSolicitudSerializado').val(data.d[1]);
