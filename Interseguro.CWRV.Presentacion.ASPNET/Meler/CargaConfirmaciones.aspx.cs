...
{
    public partial class CargaConfirmaciones : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (Utilitarios.ValidarPermiso(HttpContext.Current.Session["OpcionesSistema"], Enums.OpcionesSistema.CargaConfirmaciones))
                {
                    ...
                    CargarCombobox(ConfirmacionEnviada, comboSiNo);
                }
                else
                {
        ...
        [WebMethod]
        public static Respuesta CargarSolicitudesConfirmacion(string tokenUsuario, string fechaInicio, string fechaFin, string enviado)
        {
            using (log4net.NDC.Push(MethodBase.GetCurrentMethod().Name))
            {
                ...
                try
                {
                    ...
                    {
                        var pagina = new Page();
                        var control = (TablaSolicitudesConfirmacion)pagina.LoadControl("~/Controles/TablaSolicitudesConfirmacion.ascx");
                        ...
                        if (Utilitarios.ValidarPermiso(HttpContext.Current.Session["OpcionesSistema"], Enums.OpcionesSistema.CargaConfirmaciones))
                        {
                            ...
                            List<Solicitud> solicitudes = servicioCotizador.ListarConfirmaciones(dFechaInicio, dFechaFin, enviado[0]);
        ...
        [WebMethod]
        public static Respuesta ModificarSolicitudesConfirmacion(string tokenUsuario, string[] solicitudes)
        {
            using (log4net.NDC.Push(MethodBase.GetCurrentMethod().Name))
            {
                Respuesta respuesta = new Respuesta();
                try
                {
                    if (String.Equals(tokenUsuario, (string)HttpContext.Current.Session["TokenUsuario"]))
                    {
                        if (Utilitarios.ValidarPermiso(HttpContext.Current.Session["OpcionesSistema"], Enums.OpcionesSistema.CargaConfirmaciones))
                        {
                            ...
                            HttpContext.Current.Session["XMLCargaConfirmaciones"] = respuesta.Contenido;
                        }
                        else
                        {
                            log.Warn(String.Format("Usuario intentó acceder a una opción a la que no tiene privilegios [{0}].",
