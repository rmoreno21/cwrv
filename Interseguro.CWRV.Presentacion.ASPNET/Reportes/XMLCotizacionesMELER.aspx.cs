...
{
    public partial class XMLCotizacionesMELER : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            using (log4net.NDC.Push(MethodBase.GetCurrentMethod().Name))
            {
                try
                {
                    string nombreArchivo = String.Format("CargaCotizaciones_{0}.xml", DateTime.Now.ToString("yyyyMMdd"));
                    Response.ContentType = "text/xml";
                    Response.AddHeader("Content-Disposition", string.Format("attachment;filename={0}", nombreArchivo));
                    Response.Clear();
                    Response.Write(Session["XMLCargaCotizaciones"]);
                    Response.End();
                }
